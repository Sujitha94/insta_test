...
const FlowInstaxBot: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTemplateMessages = async () => {
      if (!tenentId) return;
      try {
        const response = await axios.get(`/api/chatflowwelcomeroute/template-messages/${tenentId}`);
  ...
  useEffect(() => {
    const fetchWelcomePageData = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await axios.get(`/api/chatflowwelcomeroute/welcome-page/${tenentId}`);
        if (response.data.success && response.data.data) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(`/api/chatflowwelcomeroute/welcome-page`, formData, {
    ...
    <div className="p-6 pb-28 bg-gray-100 min-h-screen">
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 bg-white rounded-lg shadow-md p-6 flex flex-col">
          <h2 className="text-lg font-semibold mb-6">Create Chatflow Welcome Message</h2>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[9999] p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full border border-gray-200">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-gray-600 mb-6 leading-relaxed">Chatflow Welcome message saved successfully!</p>
              <button
